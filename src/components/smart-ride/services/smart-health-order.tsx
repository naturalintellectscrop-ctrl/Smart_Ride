      ...
      <div className="min-h-screen bg-[#f8f9fa] pb-28">
        ...
        <header className="sticky top-0 z-40 bg-white border-b border-[#bec9bf]/30 px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <h1 className="text-lg font-semibold text-[#191c1d] truncate">{selectedFacility.name}</h1>
              <p className="text-sm text-[#6f7a71] truncate">{selectedFacility.type}</p>
            </div>
          </div>
        </header>
        ...
        <div className="px-4 pt-4">
          ...
          <div className="flex items-center gap-4 mb-4">
            <div className="flex items-center gap-1 bg-white px-3 py-1.5 rounded-full">
              ...
              <span className="text-[#191c1d] font-medium">{selectedFacility.rating}</span>
              <span className="text-[#6f7a71] text-sm">({selectedFacility.reviews})</span>
            </div>
          ...
          {/* Products */}
          <h3 className="text-[#191c1d] font-semibold mb-3">Available Products</h3>
          <div className="space-y-3">
            {filteredProducts.map((product) => {
              const cartItem = cart.items.find(c => c.id === product.id);
              return (
                <Card key={product.id} className="bg-white border-[#bec9bf]/30">
                  <CardContent className="p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <p className="text-[#191c1d] font-medium">{product.name}</p>
                        ...
                        <p className="text-[#6f7a71] text-sm mt-1">{product.description}</p>
                        <p className="text-[#005f3a] font-semibold mt-2">UGX {product.price.toLocaleString()}</p>
                      </div>
                      ...
                      {cartItem ? (
                        ...
                        <button
                          onClick={() => handleAddToCart(product)}
                          className="w-10 h-10 rounded-full bg-[#98f6be]/30 flex items-center justify-center hover:bg-rose-500/30 transition-colors"
                        >
        ...
        {cartCount > 0 && (
          <div className="fixed bottom-0 left-0 right-0 p-4 bg-[#f8f9fa]/95 backdrop-blur-lg border-t border-[#bec9bf]/30 max-w-md mx-auto">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 bg-[#98f6be]/30 rounded-xl flex items-center justify-center relative">
                <Pill className="h-6 w-6 text-[#005f3a]" />
    ...
    <div className="min-h-screen bg-[#f8f9fa] pb-20">
      ...
      <header className="sticky top-0 z-40 bg-white border-b border-[#bec9bf]/30 px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-[#191c1d]">Smart Health</h1>
            <p className="text-sm text-[#6f7a71]">Order medicines & healthcare products</p>
          </div>
        </div>
      </header>
      ...
      <div className="px-4 pt-4">
        ...
        <Card className="bg-rose-500/10 border-rose-500/30 mb-4">
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[#005f3a] font-medium text-sm">Prescription Required?</p>
                <p className="text-[#6f7a71] text-xs">Some medicines require a valid prescription. Upload it during checkout.</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        {/* Facilities */}
        <h3 className="text-[#191c1d] font-semibold mb-3">Health Facilities</h3>
        <div className="space-y-4">
          {healthFacilities.map((facility) => (
            <Card
              key={facility.id}
              className={cn(
                "bg-white border-[#bec9bf]/30 overflow-hidden cursor-pointer transition-all active:scale-[0.98]",
                facility.isOpen ? "hover:border-[#005f3a]/30" : "opacity-50"
              )}
            ...
            >
              <CardContent className="p-4">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <h3 className="text-[#191c1d] font-semibold">{facility.name}</h3>
                      {!facility.isOpen && (
                        <span className="text-xs bg-gray-500/20 text-[#6f7a71] px-2 py-0.5 rounded-full">Closed</span>
                      )}
                    </div>
                    <p className="text-[#6f7a71] text-sm">{facility.type}</p>
                  </div>
                  <div className="flex items-center gap-1 bg-rose-500/10 px-2 py-1 rounded">
                    <Heart className="h-3 w-3 text-[#005f3a] fill-rose-400" />
                    <span className="text-sm font-medium text-[#005f3a]">{facility.rating}</span>
                  </div>
                </div>
