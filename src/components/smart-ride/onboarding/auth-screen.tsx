...
import { signInWithGoogle, handleGoogleRedirectResult, GoogleSignInResult } from '@/lib/firebase/firebase-service';

interface AuthScreenProps {
  onBack: () => void;
  onAuthSuccess: (userData: { phone?: string; email?: string; name: string; userId?: string; token?: string }) => void;
}
...
export function AuthScreen({ onBack, onAuthSuccess }: AuthScreenProps) {
  ...
  // Handle Google auth success - defined before use
  const handleGoogleAuthSuccess = async (result: GoogleSignInResult, token: string) => {
    if (!result.user?.idToken) {
      setFirebaseError('Failed to get authentication token');
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        setFirebaseError(data.error || 'Authentication failed');
      }
    } catch (err) {
      setFirebaseError('Failed to connect to server');
    }
  ...
  useEffect(() => {
    const checkRedirectResult = async () => {
      ...
      if (result?.success && result.user) {
        await handleGoogleAuthSuccess(result, token || '');
      }
    };
    checkRedirectResult();
  }, []);
  ...
  const handlePhoneSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/auth/send-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ phone: `+256${phone}` }),
      });
  ...
  const handleOtpSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/auth/verify-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ phone: `+256${phone}`, otp }),
      });
      ...
      if (data.success) {
        ...
        setTimeout(() => {
          onAuthSuccess({ 
            phone: `+256${phone}`, 
            name: data.user?.name || `User ${phone.slice(-4)}`,
            userId: data.user?.id,
            token: data.tokens?.accessToken,
          });
        }, 1000);
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (result.success && result.user) {
        ...
        setFirebaseError(result.error || 'Google sign-in failed');
        setIsLoading(false);
      }
    } catch (err) {
