...
import { MerchantRegistration } from './onboarding/merchant-registration';
import { HealthProviderRegistration } from './onboarding/health-provider-registration';
...
export function SmartRideApp() {
  ...
  const handleRoleSelect = (role: UserRole) => {
    ...
    if (role === 'RIDER') {
      ...
      // For merchants, go to merchant registration
      setOnboardingStep('merchant-registration');
    } else if (role === 'HEALTH_PROVIDER') {
      // For health providers, go to health provider registration
      setOnboardingStep('health-provider-registration');
    } else {
  ...
  // Handle merchant registration complete
  const handleMerchantRegistrationComplete = (userData: Partial<User>) => {
    updateUser(userData);
    setOnboardingStep('pending-approval');
  };

  // Handle merchant registration back
  const handleMerchantRegistrationBack = () => {
    setOnboardingStep('role-selection');
  };

  // Handle health provider registration complete
  const handleHealthProviderRegistrationComplete = (userData: Partial<User>) => {
    updateUser(userData);
    setOnboardingStep('pending-approval');
  };

  // Handle health provider registration back
  const handleHealthProviderRegistrationBack = () => {
    setOnboardingStep('role-selection');
  };
  ...
  const renderScreen = () => {
    switch (onboardingStep) {
      ...
      case 'rider-registration':
        return (
          <RiderRegistration 
            riderRole={user?.riderRoleType || 'SMART_BODA'}
      ...
      case 'merchant-registration':
        return (
          <MerchantRegistration 
            onBack={handleMerchantRegistrationBack}
            onComplete={handleMerchantRegistrationComplete}
          />
        );

      case 'health-provider-registration':
        return (
          <HealthProviderRegistration 
            onBack={handleHealthProviderRegistrationBack}
            onComplete={handleHealthProviderRegistrationComplete}
          />
        );
      ...
      case 'dashboard':
        ...
        switch (user.role) {
          ...
          case 'HEALTH_PROVIDER':
            ...
            if (!user.providerStatus || user.providerStatus === 'PENDING' || user.providerStatus === 'DOCUMENTS_REQUESTED') {
              return (
                <PendingApproval 
                  user={user}
                />
