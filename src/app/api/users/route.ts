...
import { z } from 'zod';
import { requireAdmin, requireSuperAdmin, isAdmin } from '@/lib/auth/guards';
 ...
 * SECURITY: Super Admin only for creating admin accounts
 */
export async function POST(request: NextRequest) {
  try {
    ...
    // SECURITY: Require super admin for creating admin accounts
    const adminRoles: UserRole[] = ['ADMIN', 'SUPER_ADMIN', 'OPERATIONS_ADMIN', 'COMPLIANCE_ADMIN', 'FINANCE_ADMIN'];
    if (adminRoles.includes(validatedData.role as UserRole)) {
      const authResult = requireSuperAdmin(request);
      if (!authResult.success) {
        return NextResponse.json(
          { success: false, error: 'Only Super Admins can create admin accounts' },
          { status: 403 }
        );
      }
    } else {
