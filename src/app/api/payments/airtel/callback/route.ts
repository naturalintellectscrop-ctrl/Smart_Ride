/**
 * Airtel Money Callback Handler (Alternate Route)
 * POST /api/payments/airtel/callback
 *
 * Delegates to the same logic as /api/payments/airtel-callback
 * to avoid code duplication. Both routes exist because different
 * Airtel API configurations may use different callback URLs.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Airtel Money Callback received (alternate route):', JSON.stringify(body, null, 2));
    ...
    try {
      await db.auditLog.create({
        data: {
          ...
          description: `Airtel Money callback (alternate route): payment ${payment.paymentReference} → ${newStatus}`,
    ...
    console.error('Airtel Money callback error (alternate route):', error);
